...
class ArgParser:
  ...
  def __init__(self):
    self.parser = argparse.ArgumentParser(prog=__name__,
                                          description="Randomly generates a character.")
    self.parser.add_argument("definition", help="Configuration file for character generation.")
    self.parser.add_argument("dbs", help="Path to configuration files referenced in the configuration file.", metavar="files", nargs="+")
    self.parser.add_argument("-l", "--log", help="Sets the log level.", default="info", metavar="LEVEL", dest="loglevel", choices=["debug", "info", "warning", "error"])
    self.parser.add_argument("--debug", help="Sets debug mode (equivalent to '--log debug').", action="store_const", dest="loglevel", const="debug")
...
def main():
  ...
  CharacterGenerator(dbs, definition).generate()
