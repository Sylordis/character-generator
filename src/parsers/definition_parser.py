...
class DefinitionParser:
    ...
    def parse(self, definition_file: Path) -> CharacterDefinition | None:
        ...
        with open(definition_file, "r") as file:
            ...
            if (
                YAMLtags.CHARACTER_DEFINITION_ROOT in data
                and YAMLtags.CHARACTER_DEFINITION in data[YAMLtags.CHARACTER_DEFINITION_ROOT]
            ):
                yaml_def: dict = data.get(YAMLtags.CHARACTER_DEFINITION_ROOT).get(
                    YAMLtags.CHARACTER_DEFINITION
                )
                for yaml_entry in yaml_def:
                    ...
                    for entry in yaml_entry.get(YAMLtags.CHARACTER_DEFINITION_VALUES):
                        definition.traits.append(CharacterDefinitionTrait(db, entry))
            else:
